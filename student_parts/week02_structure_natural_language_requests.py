...
#     StructuredRequestBatch, build_week02_agent(), Week 3 이상 bridge 흐름을 확인합니다.
...
#   - extract_structured_request()와 extract_schedule_request()는 Week 3 이상에서 재사용되는
#     구조화 bridge입니다. Week 2 파일에 있지만 Week 2 agent에 공개되는 tool은 아닙니다.
#
# 구현 대상
#   1회차. Week 1 tool JSON 기반 구조화 agent 세로 슬라이스
...
#      - StructuredRequestBatch를 최소 구현하고 build_week02_agent()에
#        response_format=StructuredRequestBatch를 연결해 ./run.sh --week2가 동작하게 합니다.
...
#
#   2회차. Week 3 연결 bridge
#      - _coerce_structured_request, extract_structured_request, extract_schedule_request를 확인합니다.
#      - extract_structured_request는 agent를 새로 만들지 않고 structured LLM 호출로 자연어 또는
#        JSON 문자열을 StructuredRequest 하나로 검증/구조화합니다.
#      - extract_schedule_request는 Week 3 이상 agent가 저장/조율 전에 호출하는 LangChain bridge tool입니다.
#      - 요청이 하나뿐이어도 Week 2 agent의 structured_response에는 StructuredRequest 하나를 담습니다.
#      - 여러 일정/할 일/알림 의도가 한 문장에 섞이면 여러 StructuredRequest로 나눕니다.
#      - Week 1 personal_create_schedule은 이미 분해된 인자로 임시 일정을 생성하고,
#        Week 2 agent는 그 JSON payload를 읽어 저장 가능한 구조로 최종 변환한다는 차이를 비교합니다.
#
...
#      - Week 2 agent에는 노출되지 않으며, Week 3부터 공개 tool 목록에 들어갑니다.
...
#     response_format=StructuredRequestBatch가 설정된 agent를 만들고 재사용합니다. build_week_agent()는 실행기가 찾는 표준 entry point입니다.
