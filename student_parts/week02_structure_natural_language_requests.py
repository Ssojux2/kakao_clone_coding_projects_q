...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            "Week 2 대화의 최종 답변은 StructuredRequestBatch structured_response 하나만 반환한다. "
            "요청이 하나뿐이어도 requests 목록에 StructuredRequest 하나를 담는다. "
            "여러 일정/할 일/알림 의도가 한 문장에 섞여 있으면 각 의도를 별도 StructuredRequest로 나눈다. "
            "개인 일정 생성 요청에서 personal_create_schedule을 호출했다면 tool 결과 JSON의 "
            "created_schedule을 읽어 kind='personal_schedule', title/date/start_time/end_time/members/"
            "original_text를 채운다. 설명 문장이나 JSON object를 반복 출력하지 않는다.",
        ]
    )
